...
class ipStatistics(object):
  ...
  def getIpStatisticsByDate(self, dateFrom, dateTo):
    ...
    result = list(pgConn.execute_select("SET TIME ZONE 'CET'; SELECT accessed::date, sourceidp, service, userid, ip, ipversion FROM {0} WHERE accessed BETWEEN  '{1}' AND '{2}'".format(ipStatistics.IPSTATISTICSTABLE, dateFrom, dateTo)))
    data = []
    for row in result:
  ...
  def getAllIpStatistics(self):
    ...
    pgConn = sourcePgConnector()
    result = list(pgConn.execute_select("SET TIME ZONE 'CET'; SELECT accessed::date, sourceidp, service, userid, ip, ipversion FROM {0} WHERE accessed <= '{1}'".format(ipStatistics.IPSTATISTICSTABLE, dateTo)))
    data = []
    for row in result:
      ipData = ipStatistics(row[0], row[1], row[2], row[3], row[4], row[5])
      data.append(ipData)
